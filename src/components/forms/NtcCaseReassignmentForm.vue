...
const fillTestData = async () => {
  ...
  try {
    // 填充所有测试数据
    Object.keys(NTC_CASE_REASSIGNMENT_TEST_DATA).forEach(key => {
      formStore.updateNtcCaseReassignmentForm(key, NTC_CASE_REASSIGNMENT_TEST_DATA[key])
    })

    // 处理 Trial Date 特殊逻辑
    const testTrialDate = NTC_CASE_REASSIGNMENT_TEST_DATA.TrialDate
    ...
    console.log('NTC Case Reassignment 测试数据已填充:', formStore.ntcCaseReassignmentForm)
