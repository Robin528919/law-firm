...
import {VALIDATION_RULES, DECL_TO_CONT_CMC_TEST_DATA, API_CONFIG} from '@/utils/constants'
...
const formData = formStore.declToContCmcForm
...
const fillTestData = async () => {
  ...
  try {
    // 填充所有测试数据
    Object.keys(DECL_TO_CONT_CMC_TEST_DATA).forEach(key => {
      if (Object.prototype.hasOwnProperty.call(formData, key)) {
        if (key !== 'PlaintiffPlurality1' && key !== 'DefendantPlurality1' && key !== 'ExecutedDate') {
          // 不填充自动计算的字段
          formData[key] = DECL_TO_CONT_CMC_TEST_DATA[key]
        }
      }
    })

    // 处理 Trial Date 特殊逻辑
    const testTrialDate = DECL_TO_CONT_CMC_TEST_DATA.TrialDate
    ...
    console.log('DECL TO CONT CMC 测试数据已填充:', formData)
